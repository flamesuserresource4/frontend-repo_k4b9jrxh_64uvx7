...
const fieldVariants = {
  focus: { scale: 1.01, boxShadow: "0 0 0 4px rgba(16, 185, 129, 0.15)" },
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative py-28">
      ...
      <motion.div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 -top-20 h-20 bg-gradient-to-b from-teal-500/10 to-transparent"
        initial={{ opacity: 0 }}
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          ...
          >
            Tell us about your home or business and we'll design the best system for you.
          </motion.p>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          >
            <form onSubmit={onSubmit} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <motion.div variants={fieldVariants} whileFocus="focus" animate="blur">
              ...
              <motion.button
                ...
                whileHover={{ y: -2, boxShadow: "0 10px 30px rgba(16,185,129,0.25)" }}
                whileTap={{ scale: 0.98 }}
                className="rounded-xl bg-emerald-500 px-6 py-3 text-white font-semibold hover:bg-emerald-400 transition-colors"
              >
          ...
          <motion.div
            ...
            className="rounded-2xl border border-white/10 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 p-6"
          >
            <div className="space-y-4 text-slate-200">
              ...
              {/* small animated stats */}
              <div className="mt-6 grid grid-cols-3 gap-3 text-center">
                {["4.9★", "10k+", "24/7"].map((stat, i) => (
                  <motion.div key={stat} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 * i }} className="rounded-xl border border-white/10 bg-slate-900/40 px-3 py-2">
                    <div className="text-white font-semibold">{stat}</div>
                    <div className="text-xs text-slate-400">{i === 0 ? "Rating" : i === 1 ? "Installs" : "Support"}</div>
                  </motion.div>
                ))}
              </div>
            </div>
